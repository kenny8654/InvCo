...
class InverseCookingModel(nn.Module):
    ...
    def forward(self, img_inputs, captions,
        ...
        outputs, ids = self.recipe_decoder(captions, img_features)
    ...
    def sample(self, img_inputs, greedy=True, temperature=1.0, beam=-1, true_ingrs=None):
        ...
        if not self.recipe_only:
            ingr_ids, ingr_probs = self.ingredient_decoder.sample(greedy=True, temperature=temperature,
                                                                  beam=-1,
        ...
        # print('Model sample to transformer sample')

        ids, probs = self.recipe_decoder.sample(greedy, temperature, beam, img_features, 0,
                                                last_token_value=1)
